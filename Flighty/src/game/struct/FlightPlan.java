...
public class FlightPlan {
	 ...
	 *            Required by Slick2D
	 * @param entryPoint
	 *            The entry point for a flight i.e. the first waypoint the plane
	 *            must fly through
	...
	public ArrayList<Point> buildRoute(Game currentGame, Point entryPoint) {
		...
		if (!currentGame.getListOfWaypoints().isEmpty()
				&& !currentGame.getListOfExitPoints().isEmpty()) {
			...
			// Makes sure that the entrypoints and the exitpoints are not on the
			// same side
			while (exitpointAdded == false) {
				...
				/*
				 * If the entrypoint is the airport then the runway points are
				 * added to the plan and the airport removed.
				 */
				if (tempListOfExitPoints.get(ExitPointIndex).equals(
						currentGame.getAirport())) {
					tempRoute.add(currentGame.getAirport()
