...
public class LevelSelect extends GenericWindow {
	...
	private void drawButtons(GameContainer gameContainer, StateBasedGame game) {
		...
		// Get the text width
		int titleTextWidth = font.getWidth(titleText);
		int level1Width = font.getWidth(level1Text);
		int level2Width = font.getWidth(level2Text);
		int level3Width = font.getWidth(level3Text);
		int difficulty1Width = font.getWidth(difficulty1);
		int difficulty2Width = font.getWidth(difficulty2);
		int difficulty3Width = font.getWidth(difficulty3);
		...
		int[] level2ImagePos = {
				(gameContainer.getWidth() - levelImageSize[0]) / 2, 175 };
		int[] level3ImagePos = {
				(gameContainer.getWidth() - levelImageSize[0] - 110), 175 };
		...
		if ((x >= (level1ImagePos[0] - tolerance))
			...
			if (clicked) {
				...
				level1ImageCurrent = level1ImageHover;
			}
		} else {
		...
		if (unlock2 && (x >= (level2ImagePos[0] - tolerance))
			...
			if (clicked) {
				...
				// Apply hover image if cursor is hovering
				level2ImageCurrent = level2ImageHover;
			}
		} else {
			// Draw the level2 image
			level2ImageCurrent = level2Image;
		}
		...
		if (unlock3 && (x >= (level3ImagePos[0] - tolerance))
			...
			if (clicked) {
				...
				// Apply hover image if cursor is hovering
				level3ImageCurrent = level3ImageHover;
			}
		} else {
			// Draw the level3 image
			level3ImageCurrent = level3Image;
		}
		...
		arrowIconShaded.draw(mainMenuTextPos[0] + mainMenuWidth + 20,
				mainMenuTextPos[1]);
		arrowIcon.draw(mainMenuTextPos[0] + mainMenuWidth + 18,
				mainMenuTextPos[1] - 2);
	}
	...
	public void init(GameContainer gameContainer, StateBasedGame game)
		...
		InputStream level1Stream = this.getClass().getResourceAsStream(
				"/resources/maps/Map1Small.png");
		InputStream level1HoverStream = this.getClass().getResourceAsStream(
				"/resources/maps/Map1SmallHover.png");

		InputStream level2Stream = this.getClass().getResourceAsStream(
		...
		this.level1Image = new Image(level1Stream, "Level 1 Small", false);
	...
	public void enter(GameContainer gameContainer, StateBasedGame game)
		...
		unlock2 = sf.getLevel2Unlock();
		unlock3 = sf.getLevel3Unlock();
	}
	...
	/**
	 * @return image used for level 1 preview (when hovered over)
	 */
	...
	/**
	 * @return image used for level 2 preview (when hovered over)
	 */
	...
	/**
	 * @return image used for level 3 preview (when hovered over)
	 */
	...
	/**
	 * @param level1Image
	 *            the new image to use for level 1 preview
	 */
	 ...
	 * /**
	 * 
	 * @param level1ImageHover
	 *            the new image to use for level 1 preview (when hovered over)
	 */
	...
	/**
	 * @param level2Image
	 *            the new image to use for level 2 preview
	 */
	...
	/**
	 * @param level2ImageHover
	 *            the new image to use for level 2 preview (when hovered over)
	 */
